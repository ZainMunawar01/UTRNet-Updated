...
class CTCLabelConverter(object):
    ...
    def encode(self, text, batch_max_length=25):
        ...
        batch_text = torch.zeros(len(text), batch_max_length, dtype=torch.int64)
        for i, t in enumerate(text):
            text = list(t)
            text = [self.dict[char] for char in text]
            batch_text[i][:len(text)] = torch.tensor(text, dtype=torch.int64)
        return (batch_text, torch.tensor(length, dtype=torch.int32))
...
class CTCLabelConverterForBaiduWarpctc(object):
    ...
    def encode(self, text, batch_max_length=25):
        ...
        return (torch.tensor(text, dtype=torch.int32), torch.tensor(length, dtype=torch.int32))
    ...
    def encode(self, text, batch_max_length=25):
        ...
        for i, t in enumerate(text):
            ...
            try:
                ...
                continue
            batch_text[i][1:1 + len(text)] = torch.tensor(text, dtype=torch.int64)  # batch_text[:, 0] = [GO] token
        return (batch_text, torch.tensor(length, dtype=torch.int32))
