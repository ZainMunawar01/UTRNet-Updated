...
def validation(model, criterion, evaluation_loader, converter, opt, device):
    ...
    for i, (image_tensors, labels) in enumerate(tqdm(evaluation_loader)):
        ...
        length_for_pred = torch.tensor([opt.batch_max_length] * batch_size, dtype=torch.int32, device=device)
        text_for_pred = torch.zeros(batch_size, opt.batch_max_length + 1, dtype=torch.int64, device=device)
        ...
        if 'CTC' in opt.Prediction:
            ...
            preds_size = torch.tensor([preds.size(1)] * batch_size, dtype=torch.int32)
