...
class Attention(nn.Module):
    ...
    def _char_to_onehot(self, input_char, onehot_dim=38):
        ...
        one_hot = torch.zeros(batch_size, onehot_dim, dtype=torch.float32, device=self.device)
    ...
    def forward(self, batch_H, text, is_train=True, batch_max_length=25):
        ...
        output_hiddens = torch.zeros(batch_size, num_steps, self.hidden_size, dtype=torch.float32, device=self.device)
        ...
        if is_train:
            ...
            targets = torch.zeros(batch_size, dtype=torch.int64, device=self.device)  # [GO] token
